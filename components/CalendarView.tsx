...
import { ChevronLeftIcon, ChevronRightIcon, ClockIcon, UsersIcon, FileTextIcon, PlusIcon, MapPinIcon, CalendarIcon, DownloadIcon } from '../constants';
...
const CalendarHeader: React.FC<CalendarHeaderProps> = ({ currentDate, viewMode, onPrev, onNext, onToday, onViewModeChange, onInfoClick, onExportIcs }) => (
     <div className="flex-shrink-0 p-4 flex items-center justify-between border-b border-brand-border">
        ...
        <div className="flex items-center gap-2">
            ...
            <button onClick={onExportIcs} className="button-secondary px-3 py-1.5 text-sm inline-flex items-center gap-2">
...
const MonthView: React.FC<MonthViewProps> = ({ currentDate, daysInMonth, eventsByDate, profile, onDayClick, onEventClick }) => (
    <div className="grid grid-cols-7 flex-grow h-full calendar-grid">
        ...
        {daysInMonth.map((day, i) => {
            ...
            return (
                <div key={i} onClick={() => onDayClick(day)} className={`relative border-b border-l border-brand-border p-1 sm:p-1.5 flex flex-col min-h-[6rem] sm:min-h-[8rem] ${isCurrentMonth ? 'bg-brand-surface' : 'bg-brand-bg'} cursor-pointer hover:bg-brand-input transition-colors`}>
...
const EventPanel: React.FC<EventPanelProps> = ({ isOpen, mode, selectedEvent, eventForm, teamMembers, profile, onClose, onSetMode, onFormChange, onTeamChange, onSubmit, onDelete }) => {
    const googleCalendarLink = useMemo(() => {
        ...
        const toUtcFormat = (date: Date) => {
            return date.toISOString().replace(/[-:.]/g, '').slice(0, 15) + 'Z';
        };

        const toAllDayFormat = (date: Date) => {
        ...
        const hasTime = selectedEvent.startTime;
    
        if (hasTime) {
            const [startHours, startMinutes] = selectedEvent.startTime!.split(':').map(Number);
            startDate.setHours(startHours, startMinutes, 0, 0);
            
            if (selectedEvent.endTime) {
                endDate = new Date(selectedEvent.date);
                const [endHours, endMinutes] = selectedEvent.endTime.split(':').map(Number);
                endDate.setHours(endHours, endMinutes, 0, 0);
            } else {
                endDate = new Date(startDate.getTime() + (2 * 60 * 60 * 1000)); // Default 2 hours
            }

            const dates = `${toUtcFormat(startDate)}/${toUtcFormat(endDate)}`;
            const details = `Proyek: ${selectedEvent.projectName}\nKlien: ${selectedEvent.clientName || 'Acara Internal'}\nCatatan: ${selectedEvent.notes || 'Tidak ada'}`;
            return `https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(selectedEvent.projectName)}&dates=${dates}&details=${encodeURIComponent(details)}&location=${encodeURIComponent(selectedEvent.location || '')}`;
            ...
            const dates = `${toAllDayFormat(startDate)}/${toAllDayFormat(endDate)}`;
            const details = `Proyek: ${selectedEvent.projectName}\nKlien: ${selectedEvent.clientName || 'Acara Internal'}\nCatatan: ${selectedEvent.notes || 'Tidak ada'}`;
            return `https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(selectedEvent.projectName)}&dates=${dates}&details=${encodeURIComponent(details)}&location=${encodeURIComponent(selectedEvent.location || '')}`;
        }
    }, [selectedEvent]);
        ...
        <div className="overflow-y-auto flex-1">
            {mode === 'detail' && selectedEvent ? (
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="p-6 border-t border-brand-border flex flex-wrap gap-2">
                        <button onClick={() => onSetMode('edit')} className="button-primary flex-1">Edit Detail Acara</button>
                        <a href={googleCalendarLink} target="_blank" rel="noopener noreferrer" className="button-secondary inline-flex items-center justify-center gap-2 flex-1">
                ...
                <div className="p-6">
                    ...
                    <form onSubmit={onSubmit} className="space-y-4">
                        ...
                        <div className="input-group"><input type="text" id="location" name="location" value={eventForm.location} onChange={onFormChange} className="input-field" placeholder=" "/><label htmlFor="location" className="input-label">Lokasi</label></div>
...
export const CalendarView: React.FC<CalendarViewProps> = ({ projects, setProjects, teamMembers, profile }) => {
    ...
    const initialFormState = useMemo(() => ({
        ...
        startTime: '', endTime: '', notes: '', team: [] as AssignedTeamMember[], image: '', location: '',
    }), [profile.eventTypes]);
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (selectedEvent) { // Editing
            ...
            const newEvent: Project = {
                ...
                packageName: '', packageId: '', addOns: [], deadlineDate: '', 
    ...
    const handleExportIcs = () => {
        ...
        const futureEvents = projects.filter(p => new Date(p.date) >= new Date());
        if (futureEvents.length === 0) {
            alert('Tidak ada acara mendatang untuk diekspor.');
            return;
        }

        const icsEvents = futureEvents.map(event => {
            ...
            const description = `Proyek: ${event.projectName}\\nKlien: ${event.clientName}\\nLokasi: ${event.location}\\nCatatan: ${event.notes || 'Tidak ada'}`.replace(/\n/g, '\\n');

            return [
                'BEGIN:VEVENT',
                `UID:${event.id}@venapictures.com`,
        ...
        const icsFileContent = [
            'BEGIN:VCALENDAR',
            'VERSION:2.0',
            'PRODID:-//Vena Pictures//Calendar Exporter v1.0//EN',
            icsEvents,
            'END:VCALENDAR'
        ].join('\r\n');
        ...
        link.setAttribute("download", "vena_pictures_calendar.ics");
            ...
            <Modal isOpen={isInfoModalOpen} onClose={() => setIsInfoModalOpen(false)} title="Panduan Halaman Kalender">
                <div className="space-y-4 text-sm text-brand-text-primary">
                    ...
                    <ul className="list-disc list-inside space-y-2">
                        ...
                        <li><strong>Integrasi Kalender:</strong> Gunakan tombol "Export .ics" untuk mengunduh file kalender yang bisa diimpor ke Google Calendar atau Apple Calendar. Gunakan tombol "Tambah ke GCal" di detail acara untuk menambahkan acara individual.</li>
                    </ul>
                </div>
            </Modal>
        </div>
