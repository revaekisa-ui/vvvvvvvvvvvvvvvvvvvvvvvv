...
import { Profile, Transaction, Project, User, ViewType, ProjectStatusConfig, SubStatusConfig, PublicPageConfig, Package, TransactionType, SOP } from '../types';
...
const ProjectStatusManager: React.FC<{
...
}> = ({ config, onConfigChange, projects, setProjects }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (modalMode === 'add') {
            ...
            const oldStatusName = selectedStatus.name;
            const newStatusName = form.name;
            ...
            if (oldStatusName !== newStatusName) {
                setProjects(prevProjects => prevProjects.map(p => 
                    p.status === oldStatusName ? { ...p, status: newStatusName } : p
                ));
            }
        }
        handleCloseModal();
...
const Settings: React.FC<SettingsProps> = ({ profile, setProfile, transactions, setTransactions, projects, setProjects, packages, setPackages, sops, setSops, users, setUsers, currentUser, showNotification }) => {
    ...
    const permissionGroups = {
        ...
        'Operasional': [ViewType.TEAM, ViewType.FINANCE, ViewType.CALENDAR, ViewType.SOCIAL_MEDIA_PLANNER],
        'Konfigurasi & Aset': [ViewType.PACKAGES, ViewType.ASSETS, ViewType.CONTRACTS, ViewType.PROMO_CODES, ViewType.SOP],
        'Laporan': [ViewType.CLIENT_REPORTS],
    };
    const navItemsMap = new Map(NAV_ITEMS.map(item => [item.view, item]));
...
const base64Promises = files.map((file: File) => toBase64(file));
    ...
    const handleAddOrUpdateIncomeCategory = () => {
        ...
        if (editingIncomeCategory) { // Update
            ...
            setTransactions(prevTxs => prevTxs.map(tx => (tx.category === editingIncomeCategory && tx.type === TransactionType.INCOME) ? { ...tx, category: newCategory } : tx));
    ...
    const handleDeleteIncomeCategory = (category: string) => {
        const isCategoryInUse = transactions.some(t => t.category === category && t.type === TransactionType.INCOME);
        if (isCategoryInUse) {
    ...
    const handleAddOrUpdateExpenseCategory = () => {
        ...
        if (editingExpenseCategory) {
            ...
            setTransactions(prevTxs => prevTxs.map(tx => (tx.category === editingExpenseCategory && tx.type === TransactionType.EXPENSE) ? { ...tx, category: newCategory } : tx));
    ...
    const handleDeleteExpenseCategory = (category: string) => {
        const isCategoryInUse = transactions.some(t => t.category === category && t.type === TransactionType.EXPENSE);
        if (isCategoryInUse) {
            alert(`Kategori "${category}" tidak dapat dihapus karena sedang digunakan dalam transaksi.`); return;
        }
        if (window.confirm(`Yakin ingin menghapus kategori "${category}"?`)) {
    ...
    const handleAddOrUpdateProjectType = () => {
        ...
        if (editingProjectType) {
            ...
            setProjects(prevProjects => prevProjects.map(p => p.projectType === editingProjectType ? { ...p, projectType: newType } : p));
    ...
    const handleAddOrUpdateEventType = () => {
        ...
        if (editingEventType) {
            ...
            setProjects(prevProjects => prevProjects.map(p => (p.clientName === 'Acara Internal' && p.projectType === editingEventType) ? { ...p, projectType: newType } : p));
    ...
    const handleAddOrUpdateSopCategory = () => {
        ...
        if (editingSopCategory) {
            ...
            setSops(prevSops => prevSops.map(s => s.category === editingSopCategory ? { ...s, category: newCat } : s));
    ...
    const handleDeleteSopCategory = (cat: string) => {
        const isUsed = sops.some(s => s.category === cat);
        if (isUsed) {
            alert(`Kategori "${cat}" tidak dapat dihapus karena sedang digunakan oleh SOP.`); return;
        }
    ...
    const handleAddOrUpdatePackageCategory = () => {
        ...
        if (editingPackageCategory) {
            ...
            setPackages(prevPackages => prevPackages.map(p => p.category === editingPackageCategory ? { ...p, category: newCat } : p));
                ...
                return (
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {users.map(user => (
                                <div key={user.id} className="p-4 bg-brand-bg rounded-xl border border-brand-border flex flex-col justify-between">
                                    <div>
                                        <div className="flex justify-between items-start">
                                            <div className="flex items-center gap-2">
                                                <p className="font-bold text-lg text-brand-text-light">{user.fullName}</p>
                                            ...
                                            <span className={`px-2 py-1 text-xs font-semibold rounded-full ${user.role === 'Admin' ? 'bg-indigo-500/20 text-indigo-400' : 'bg-blue-500/20 text-blue-400'}`}>
                                                {user.role}
                                            </span>
                                        </div>
                                        ...
                                        {user.role === 'Member' && (
                                            <p className="text-xs text-brand-text-secondary mt-2">
                                                {user.permissions?.length || 0} Izin Akses
                                            </p>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-2 mt-4 pt-4 border-t border-brand-border/50 justify-end">
                                        <button onClick={() => handleOpenUserModal('edit', user)} className="button-secondary text-sm">Edit</button>
                                        <button 
                                            onClick={() => handleDeleteUser(user.id)} 
                                            disabled={user.id === currentUser?.id}
                                            className="button-secondary text-sm !text-brand-danger !border-brand-danger/50 hover:!bg-brand-danger/10 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:!bg-transparent"
                                        >
                                            Hapus
                                        </button>
                                    </div>
                ...
                <form onSubmit={handleUserFormSubmit} className="space-y-4">
                    ...
                    {userForm.role === 'Member' && (
                        <div>
                            ...
                            <div className="space-y-4">
                                {Object.entries(permissionGroups).map(([groupName, views]) => (
                                    <div key={groupName} className="p-3 bg-brand-bg rounded-lg">
                                        <h5 className="font-semibold text-brand-text-light mb-2">{groupName}</h5>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            {views.map(view => {
                                                const navItem = navItemsMap.get(view);
                                                if (!navItem) return null;
                                                const Icon = navItem.icon;
                                                return (
                                                    <label key={view} className="flex items-center gap-2 p-2 rounded-md hover:bg-brand-input cursor-pointer transition-colors">
                                                        <input type="checkbox" checked={userForm.permissions.includes(view)} onChange={e => handlePermissionsChange(view, e.target.checked)} className="h-4 w-4 rounded" />
                                                        <Icon className="w-4 h-4 text-brand-text-secondary"/>
                                                        <span className="text-sm text-brand-text-primary">{navItem.label}</span>
                                                    </label>
                                                );
                                            })}
                                        </div>
                    ...
                    <div className="flex justify-end items-center gap-3 pt-6 border-t border-brand-border">
                        {userModalMode === 'edit' && selectedUser && selectedUser.isApproved === false && (
                            <button 
                                type="button" 
                                onClick={() => handleApproveUser(selectedUser.id)} 
                                className="button-primary !bg-brand-success hover:!bg-green-700 mr-auto"
                            >
                                Setujui Pengguna
                            </button>
                        )}
