...
{(Object.entries(teamByRole) as [string, TeamMember[]][]).map(([role, members]) => (
                                    <div key={role}>
...
const topProjectType = Object.keys(projectTypeCounts).length > 0
            ? (Object.entries(projectTypeCounts) as [string, number][])
...
return (Object.entries(statusCounts) as [string, number][]) .map(([label, value]) => {
...
(Object.entries(teamByRole) as [string, any[]][]) .map(([role, members]) => (
                                        <div key={role}>
...
const topProjectType = Object.keys(projectTypeCounts).length > 0
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formMode === 'add') {
            ...
            const updatedTeamOnProject = projectData.team;
            const otherProjectsPayments = teamProjectPayments.filter(p => p.projectId !== projectData.id);
            const existingPaymentsForProject = teamProjectPayments.filter(p => p.projectId === projectData.id);
            
            const finalPaymentsForProject = updatedTeamOnProject.map(teamMember => {
                // FIX: Property 'memberId' does not exist on type 'TeamProjectPayment'. Did you mean 'teamMemberId'?
                const existingPayment = existingPaymentsForProject.find(p => p.teamMemberId === teamMember.memberId);
                if (existingPayment) {
                    // Member was already on the team, preserve their status and update details
                    return {
                        ...existingPayment,
                        fee: teamMember.fee,
                        reward: teamMember.reward || 0,
                        teamMemberName: teamMember.name, // Handle potential name edits
                    };
                } else {
                    // This is a new member being added to the team
                    return {
            ...
            setTeamProjectPayments([...otherProjectsPayments, ...finalPaymentsForProject]);
        }
    ...
    const handleProjectDelete = (projectId: string) => {
        if (window.confirm("Apakah Anda yakin ingin menghapus proyek ini? Semua data terkait (termasuk tugas tim dan transaksi) akan dihapus, dan saldo akan dikembalikan.")) {
            const transactionsToDelete = transactions.filter(t => t.projectId === projectId);
            if (transactionsToDelete.length > 0) {
                setTransactions(prevTransactions => {
                    const remainingTransactions = prevTransactions.filter(t => t.projectId !== projectId);
                    let tempCards = [...cards];
                    transactionsToDelete.forEach(tx => {
                        if (tx.type === TransactionType.INCOME && tx.cardId) {
                            tempCards = tempCards.map(c => c.id === tx.cardId ? { ...c, balance: c.balance - tx.amount } : c);
                        } else if (tx.type === TransactionType.EXPENSE && tx.cardId) {
                            tempCards = tempCards.map(c => c.id === tx.cardId ? { ...c, balance: c.balance + tx.amount } : c);
                        }
                    });
                    setCards(tempCards);
            ...
            showNotification('Proyek dan semua data terkait berhasil dihapus.');
        }
    };
