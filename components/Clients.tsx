...
import { Client, Project, PaymentStatus, Package, AddOn, TransactionType, Profile, Transaction, ClientStatus, Card, FinancialPocket, Contract, ViewType, NavigationAction, ClientFeedback, SatisfactionLevel, PromoCode, ClientType, Notification, BookingStatus } from '../types';
                                ...
                                <div key={p.id}>
                                    ...
                                    <div className="p-4 bg-brand-bg rounded-lg flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                                        ...
                                        <div className="flex items-center gap-2 flex-shrink-0 self-start sm:self-center">
                                            ...
                                            <button onClick={() => onEditProject(p)} className="button-secondary text-sm inline-flex items-center gap-2">
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (modalMode === 'add') {
            ...
            addNotification({
                ...
                message: `Booking untuk proyek "${newProject.projectName}" dari klien ${formData.clientName} telah dibuat.`,
                icon: 'booking',
            ...
            // If client name has changed, update projects as well
            if (formData.clientName !== selectedClient.name) {
                setProjects(prevProjects => prevProjects.map(p => 
                    p.clientId === selectedClient.id ? { ...p, clientName: formData.clientName } : p
                ));
            }
            ...
            <Modal isOpen={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} title={`Detail Klien: ${clientForDetail?.name}`} size="4xl">
                <ClientDetailModal 
                    ...
                    onEditProject={(project) => {
                        if (clientForDetail) {
                            handleOpenModal('edit', clientForDetail, project);
                        }
                    }} 
